...
def oriented_weight_Euler(V, E, W):
    """
    V := set( Vertex(v) )
    E := set( Edge(u,v) )
    w := dict( Edge(u,v) : cost)
    ...
    v_odd = set([])# 奇点集合
    for v in V:
        if dev_in[v] % 2 != 0 or dev_out[v] % 2 != 0:
            v_odd.add(v)
    if len(v_odd) != 0:
        print('不存在Euler闭迹, 需要添加边')
        '求出最短路，构造完全图F'
        
        X = set([v for v in V if dev_in[v]<dev_out[v]])
        print(X)
        Y = v_odd - X
        vs, vt = 'vs', 'vt'
        # 增加收点和发点
        ...
        C_change = dict()
        for (u, v) in E:
            C_change[(u, v)] = float('inf')
            F_change[(u, v)] = W[(u,v)]

        for v in X:
            F_change[(vs, v)] = 0
            C_change[(vs, v)] = abs(dev_in[v] - dev_out[v])
        for v in Y:
            F_change[(v, vt)] = 0
            C_change[(v, vt)] = abs(dev_in[v] - dev_out[v])
    ...
    while True:
        ...
        if v_new == v_end:
            ...
            if sum(Gk.values()) == 0:
                ...
                if len([e for e in Gk if Gk[e] > 0 and e[0] == v_new]) == 0:
                    Wk.append('开始下一个欧拉环游')
...
if __name__ == '__main__':
    V = {1, 2, 3, 4, 5} 
    E = {(1, 4), (2, 1), (2, 3), (3, 1), (3, 4), (4, 5), (5, 1), (5, 2), (5, 3)}
    W = {(1, 4):4, 
        (2, 1):2, (2, 3):1,  
        (3, 1):3, (3, 4):6,
        (4, 5):2,
        (5, 1):3, (5, 2):2, (5, 3):5}
    Wk = oriented_weight_Euler(V, E, W)           
    #print(Wk)
